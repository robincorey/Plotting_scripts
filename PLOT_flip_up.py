...
def PlotLipidContact(j): 
	...
	smooth = sc.signal.savgol_filter(ydata-7.50412, 45, 1, deriv=0, delta=1, axis=-1, mode='interp', cval=0.0)
...
text_file = open("up.xvg", "r")
lines = text_file.read().split('\n')
x = np.arange()
ys = [i+x+(i*x)**2 for i in range()]
