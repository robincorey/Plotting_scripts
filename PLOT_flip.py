...
def PlotLipidContact(j): 
	...
	smooth = sc.signal.savgol_filter(ydata-##CENT##, 45, 1, deriv=0, delta=1, axis=-1, mode='interp', cval=0.0)
...
filename = 'PO4.xvg'
xdata = np.loadtxt(fname=filename, usecols=0, unpack=True)
plt.plot([##start##, ##end##], [##high##, ##high##], '0.8', lw=40)
plt.plot([##start##, ##end##], [##low##, ##low##], '0.8', lw=40)
